...
Os 26 testes de `test_arquivos.py` incluem oito variações parametrizadas da validação de extensão, uma para cada formato listado em `EXTENSOES_SUPORTADAS`. Sem o parâmetro, seriam 19 testes diretos.
...
Todos os 80 testes resultaram em `PASSED`. Não houve falhas, erros nem testes ignorados (`skipped`).
